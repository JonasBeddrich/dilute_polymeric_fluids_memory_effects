...
double phi_deg0_initial_condition(const Vector & coord)
{
    double x = coord(0); 
    double y = coord(1); 
    return 1 + cos(2 * M_PI * x) * cos(4 * M_PI * y); 
}

double phi_deg2_initial_condition(const Vector &coord)
{
    double x = coord(0); 
    double y = coord(1); 
    return cos(2 * M_PI * x) * cos(4 * M_PI * y); 
}
...
int main(int argc, char *argv[])
{
    ...
    args.AddOption(&alpha, "-a", "--alpha", "Order of the fractional derivative."); 
    args.ParseCheck();
    ...
    std::string pathname("TFNSFP_zero_vel_convergence_test");
    pathname += "_dt_" + std::to_string(setup.time_settings.dt);
    pathname += "_a_" + std::to_string(alpha);
    ParaViewDataCollection dataCollection(pathname, &pmesh);
    dataCollection.SetPrefixPath("ParaView/TFNSFP_Convergence");
    ...
    TimeFractionalFokkerPlanckSolver fp_solver(pmesh, setup.fp_settings, alpha);
    ...
    FunctionCoefficient phi_0(phi_deg0_initial_condition);
    FunctionCoefficient phi_2(phi_deg2_initial_condition);
    
    fp_solver.SetInitialConditions2D(phi_0, phi_2, phi_2, phi_2); 
